(function (doc) {
  ...
  let scriptElm =
    arr.filter(
      (item) => item.src.indexOf('dist/sdds-components-old.js') > -1
    )[0] || doc.scripts[doc.scripts.length - 1];

  const parts = scriptElm.src.split('/');
  parts.pop();
  parts.push('sdds-components-old');
  const url = parts.join('/');

  scriptElm = doc.createElement('script');
  // when stop supporting IE 11, change to /sdds-components-old.esm.js and type="module"
