...
##### Changing mode
Mode is control on a top level, using class  `sdds-mode-dark` or `sdds-mode-light` (light is default). 
...
Previously the accordion has used a class `sdds-on-white-bg` was used to determine the mode variant. We have now introduced a prop called `mode-variant` with options`primary` or
`secondary` to determine which variant is being shown. For this component `primary` is the default value and
`secondary` represents the old `sdds-on-white-bg`
...
// New ✅
<sdds-accordion mode-variant="secondary">
  <sdds-accordion-item header="Lorem ipsum" tabindex="0">
    // ...
  </sdds-accordion-item>
</sdds-accordion>
...
Rename `sdds-on-white-bg` to `sdds-mode-variant-primary`, or add `sdds-mode-variant-secondary`.
...
Previously the dropdown used the class `sdds-on-white-bg` was used to determine the mode variant. We have now introduced the prop `mode-variant` with values `primary` or
`secondary` to determine which variant is being shown. `primary` is the default value and
`secondary` represents the old `sdds-on-white-bg`
##### What action is required?
...
// New ✅
<sdds-dropdown mode-variant="secondary">
    // ...
</sdds-dropdown>
```
#### Changed class `sdds-on-white-bg` to `sdds-mode-variant-primary` in native component
...
```
#### Added prop for mode-variant and removed class `sdds-on-white-bg` on container
...
`secondary`. `primary` is the default value and
`secondary` represents the old `sdds-on-white-bg`
##### What action is required?
Replace all instances of `class="sdds-on-white-bg"` on container element with the prop `mode-variant` and use `primary` as value. directly on the component.
```jsx
// Old ❌
...
// New ✅
 <sdds-textarea mode-variant="secondary">
 </sdds-textarea>
```
...
```
#### Added prop for mode-variant and removed class `sdds-on-white-bg` on container
Previously the class `sdds-on-white-bg` was used to determine the mode variant. We have now introduced a called `mode-variant` with values `primary` or
`secondary`. `primary` is the default value and
`secondary` represents the old `sdds-on-white-bg`
##### What action is required?
Replace all instances of `class="sdds-on-white-bg"` on container element with the prop `mode-variant` directly on the component.
```jsx
// Old ❌
<div class="sdds-on-white-bg"> 
...
// New ✅
 <sdds-textfield mode-variant="secondary">
 </sdds-textfield>
```
