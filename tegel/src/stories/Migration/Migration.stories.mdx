...
Previously the accordion has used a class (`sdds-on-white-bg`) to determine the variant on the component is
used. We have now introduced a prop called `mode-variant` with options`primary` or
...
Previously the dropdown used the class (`sdds-on-white-bg`) to determine the mode variant of the component. We have now introduced the prop `mode-variant` with values `primary` or
