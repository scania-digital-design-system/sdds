...
This change affects all components/typography/spacing/grid. In the past we have used a base value of 4px
...
custom font-sizes. We have now removed these dependancies and are using pixels in our 
...
If you are not yourself using `rem` your project ideally this change should not affect you, but since it is 
something that has an affect on all components and utitlies/foundations we expect that there might be some
unforseen problems. These will have our highest prio in order to make the migration to Tegel as smooth as possible.

If you are however using `rem` in your project you would have to consider that the base value will no longer be 4 px.
