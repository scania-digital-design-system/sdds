...
[Web Component](/docs/components-dropdown--webcomponent )

#### Refactored the dropdown component.

The previous dropdown component had a webcomponent version which we felt was not
up to par with the quality of our other components. Therefore we did a full redo
and refactor of this component.

Before the filter version was a separate component that really just wrapped the 
`<sdds-dropdown>` and by doing so offered some additional features. One of
those features was the ability to pass in data via a `data` prop. Which would
then render the `<sdds-dropdown-option>` based on the data. This is now available
for all versions of the dropdown. A part from that, we have all in all created one
component, with a child component for all available version of this component.

This means that the dropdown will always have the same base, with it's children
specified as children - `<sdds-dropdown-option>`, or via it's data prop. Then
based on what features you want to use, you pass is different props.

##### What action is required?
Migrate all instances of `<sdds-dropdown>` to conform with it's updated API.

All of the dropdowns new and refactored features are outlines in the dropdown
story here in storybook. Please try it out [here](/docs/components-dropdown--webcomponent )


#### Examples - with childelements:
```jsx
// New default sdds-dropdown ✅
<sdds-dropdown>
    <sdds-dropdown-option value="option-1" label="Option-1" disabled></sdds-dropdown-option>
    <sdds-dropdown-option value="option-2" label="Option-2"></sdds-dropdown-option>
</sdds-dropdown>

// New filter sdds-dropdown ✅
<sdds-dropdown
  filter
  >
    <sdds-dropdown-option value="option-1" label="Option-1" disabled></sdds-dropdown-option>
...
The data prop expects the following JSON structure, passed to it as a string.
...
[Webcomponent](/docs/components-tabs--inline-tabs)

#### Created a new "child-element".

Previously the `<sdds-inline-tabs>` have used any native HTMLElement as it's child. This element
needed the following props `data-name`, `data-default` and `aria-disabled`. We have now created
our own child element `<sdds-inline-tab>` that should be used to wrap the tabs content. This
element has corresponding props to its predecessor:
`data-name` -> `label` 
...
Change all instances of `<sdds-inline-tabs>` with child `<div>` to use `<sdds-inline-tab>` as 
child instead.

```jsx
// Old ❌
<sdds-inline-tabs>
  <div data-name="First tab">Tab panel 1</div>
  <div data-name="Default tab" data-default="true">Tab panel 2</div>
  <div data-name="Disabled tab" aria-disabled="true">Tab panel 3</div>
</sdds-inline-tabs>

// New ✅
<sdds-inline-tabs mode-variant="primary">
  <sdds-inline-tab label="First tab">Tab panel 1</sdds-inline-tab>
  <sdds-inline-tab label="Default tab" default>Tab panel 2</sdds-inline-tab>
  <sdds-inline-tab label="Disabled tab" disabled>Tab panel 3</sdds-inline-tab>
</sdds-inline-tabs>
```
...
[Web component](/docs/components-modal--web-component)

#### Refactored how the modal is opened and removed dependency to trigger
The modal component has previously been opened via a trigger that was added on a button,
which would be the controler of the button. This has now been refactored and the modal is
now opened based on its own boolean prop, called `open`. This change is done in order to
make the component more inline with how these sort of thing are generally done. We also
dont want to assume that is is a button that controls the open/closed state of the modal
and this approach gives you more freedom on how you open your modals.

The modal will still have its public method `openModal()` which can be called on the
element to set its `open` prop to true.

##### What action is required?
All buttons that has previously been the triggers for the buttons now needs to alter the
modals `open` prop and set it to `true` in order to open the component.

```jsx
// Old ❌
<button class="sdds-btn sdds-btn-primary modal1">
  Open modal
</button>

<sdds-modal id="modal-test" size="lg" selector=".modal1" actions="static">
  <h5 class="sdds-modal-headline" slot="sdds-modal-headline">
    Headline
  </h5>
...
// New ✅
<button onclick="openMyModal()" id="my-modal-button" class="sdds-btn sdds-btn-primary">Open modal</button>

<sdds-modal id="my-modal" size="md" actions="static">
  <h5 class="sdds-modal-headline" slot="sdds-modal-headline">
    Headline
  </h5>
  <div class="sdds-modal-body" slot="sdds-modal-body">
