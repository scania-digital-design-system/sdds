...
used. We have now introduced a prop on `<accordion>` called `mode-variant` with options`primary` or
`seconday` to determine which variant is being shown. For this component `primary` is the default value and
`seconday` represents the old `sdds-on-white-bg`
...
// New ✅
<sdds-accordion mode-variant="seconday">
  <sdds-accordion-item header="Lorem ipsum" tabindex="0">
    // ...
  </sdds-accordion-item>
</sdds-accordion>
...
The classes `sdds-block__default` and `sdds-block__variant` was renamed to `sdds-block-primary` and `sdds-block-secondary` to conform with convention.

##### What action is required?

Replace all instaces of `sdds-block__default` and `sdds-block__variant` with `sdds-block-primary` and `sdds-block-secondary`.
...
// New ✅
<div class="sdds-block sdds-block-primary">
  <p>Content....</p>
</div>
<div class="sdds-block sdds-block-secondary">
  <p>Content....</p>
</div>
```
