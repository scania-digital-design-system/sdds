...
This documents inludes all the breaking changes to our components from @scania/components version 4.
...
@scania/components will have a web component alternative in @scania/tegel. This change does not change
the current @scania/components package.
...
as valid input (primary is default). If prop is given, the component is going to inherit the mode variant from its parent.
